...
def process_return():
    ...
    try:
        ...
        for item in items_in:
            ...
            # CAMBIO: Buscamos la Variante completa para tener acceso a los IDs de Tienda Nube
            variante = ProductoVariante.query.get(var_id)
            
            if variante and variante.inventario:
                # 1.1 Actualizar Local
                variante.inventario.stock_actual += 1
                
                # 1.2 Sincronizar Tienda Nube (Si el producto está vinculado)
                if variante.producto.tiendanube_id and variante.tiendanube_variant_id:
                    try:
                        ...
                        print(f"Sync TN OK: {variante.producto.nombre} (+1)")
                    except Exception as tn_error:
                        print(f"Error Sync TN (Devolución): {tn_error}")
        ...
        for item in items_out:
            ...
            # CAMBIO: Buscamos la Variante completa
            variante = ProductoVariante.query.get(var_id)
            
            if variante and variante.inventario:
                # 2.1 Actualizar Local
                variante.inventario.stock_actual -= 1
                
                # 2.2 Sincronizar Tienda Nube
                if variante.producto.tiendanube_id and variante.tiendanube_variant_id:
                    try:
                        tn_service.update_variant_stock(
                            tn_product_id=variante.producto.tiendanube_id,
                        ...
                        print(f"Error Sync TN (Salida): {tn_error}")
        ...
        if balance < 0:
            ...
            db.session.flush() # Flush para asegurar que se guarde antes del commit final

        # 4. REGISTRAR VENTA (Diferencia)
