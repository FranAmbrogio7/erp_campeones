...
import {
    ...
    ListPlus, AlertTriangle // <--- ICONOS NUEVOS
} from 'lucide-react';
...
const BudgetPage = () => {
    ...
    // --- BÚSQUEDA INFINITA (Con Filtros) ---
    useEffect(() => {
        const delaySearch = setTimeout(async () => {
            // Si no hay nada escrito Y no hay filtros seleccionados, limpiamos
            ...
            try {
                const params = {
                    ...
                    limit: 200 // Aumentamos límite para permitir listas largas
                };
    ...
    // --- NUEVO: AGREGAR TODO LO FILTRADO (MASIVO) ---
    const handleAddAllResults = () => {
        if (manualResults.length === 0) return;

        // Calcular cuántos ítems reales son (sumando variantes)
        const totalVariantes = manualResults.reduce((acc, p) => acc + p.variantes.length, 0);

        if (totalVariantes > 100) {
            if (!window.confirm(`⚠️ Estás a punto de agregar ${totalVariantes} ítems al presupuesto. ¿Continuar?`)) return;
        }
        ...
        setCart(prev => {
            ...
            manualResults.forEach(prod => {
                prod.variantes.forEach(v => {
                    const existingIdx = newCart.findIndex(i => i.id_variante === v.id_variante);

                    if (existingIdx >= 0) {
                        // Si ya existe, sumamos 1 (opcional, o lo dejamos igual)
                        const current = newCart[existingIdx];
                        newCart[existingIdx] = {
                            ...current,
                            cantidad: current.cantidad + 1,
        ...
        toast.success(`${addedCount} variantes agregadas al presupuesto`);
        setManualTerm('');
        setSelectedCat('');
        setSelectedSpec('');
    };
    ...
    const restoreBudget = (budget) => {
        if (cart.length > 0 && !window.confirm("Tienes un presupuesto en curso. ¿Reemplazarlo?")) return;
    ...
    return (
        <div className="flex flex-col md:flex-row h-full gap-4 p-4 max-w-7xl mx-auto animate-fade-in">
                ...
                <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl h-[80vh] flex flex-col overflow-hidden animate-fade-in-up">
                        <div className="p-5 border-b flex justify-between items-center bg-gray-50">
                            <h3 className="font-bold text-xl flex items-center text-gray-800"><History className="mr-2 text-blue-600" /> Historial de Presupuestos</h3>
                            <button onClick={() => setIsHistoryOpen(false)} className="p-2 hover:bg-gray-200 rounded-full"><X /></button>
                        </div>
                        <div className="flex-1 overflow-y-auto p-4">
                            {isLoadingHistory ? <div className="text-center py-10 text-gray-400">Cargando...</div> : historyList.length === 0 ? <div className="text-center py-10 text-gray-400">No hay presupuestos guardados.</div> : (
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <div className="flex gap-2">
                            <select
                                ...
                                className="flex-1 p-2 bg-gray-50 border border-gray-200 rounded-lg text-sm font-bold text-gray-700 outline-none focus:ring-2 focus:ring-blue-100"
                            >
                        ...
                        <div className="relative">
                            <div className="flex items-center border-2 border-blue-200 rounded-lg overflow-hidden focus-within:ring-4 focus-within:ring-blue-100 transition-all">
                                ...
                                <input
                                    ...
                                    placeholder="Buscar producto (o filtrar arriba)..."
                                    className="w-full p-3 outline-none font-medium"
                                />
                                {(manualTerm || selectedCat || selectedSpec) && (
                                    <button onClick={() => { setManualTerm(''); setSelectedCat(''); setSelectedSpec(''); setManualResults([]); }} className="mr-3 text-gray-400 hover:text-red-500">
                                        <X size={16} />
                                    </button>
                                )}
                            </div>
                            ...
                            {manualResults.length > 0 && (
                                <div className="absolute top-full left-0 right-0 z-50 mt-1">
                                    ...
                                    <div className="bg-blue-600 text-white p-3 rounded-t-xl flex justify-between items-center shadow-lg cursor-pointer hover:bg-blue-700 transition-colors" onClick={handleAddAllResults}>
                                        <div className="flex items-center font-bold text-sm">
                                            <ListPlus className="mr-2" />
                                            Agregar estos {manualResults.length} productos al Presupuesto
                                        </div>
                                        <span className="text-[10px] bg-blue-800 px-2 py-1 rounded">Clic para añadir todo</span>
                                    </div>

                                    {/* Resultados Flotantes (Lista debajo del botón) */}
                                    <div className="bg-white border shadow-xl rounded-b-lg max-h-80 overflow-y-auto">
                                        {manualResults.map(p => (
                                            <div key={p.id} className="p-3 border-b hover:bg-gray-50 flex gap-3 group items-start">
                                                ...
                                                <div className="flex-1">
                                                    <div className="flex justify-between items-center mb-1">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <button onClick={(e) => { e.stopPropagation(); addAllToCart(p) }} className="flex items-center bg-blue-100 text-blue-700 text-[10px] px-2 py-1 rounded hover:bg-blue-600 hover:text-white transition-colors font-bold uppercase tracking-wide">
                                                    ...
                                                    <div className="flex flex-wrap gap-2 mt-1">
                                                        {p.variantes.map(v => (
                                                            <button key={v.id_variante} onClick={(e) => { e.stopPropagation(); addToCart(p, v) }} className="text-xs px-2 py-1 rounded border hover:bg-gray-800 hover:text-white border-gray-200 text-gray-600 transition-colors">
                                                                {v.talle}
                                                            </button>
                                                        ))}
                                                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-2">
                        {cart.length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center text-gray-300">
                                ...
                                <p>Borrador vacío.</p>
                                <p className="text-xs mt-1">Usa los filtros y agrega productos masivamente.</p>
                            </div>
                        ) : (
                            cart.map(item => (
            ...
            <div className="w-full md:w-1/3 bg-white rounded-xl shadow-lg border border-gray-200 flex flex-col overflow-hidden h-full">
                <div className="bg-slate-800 text-white p-4"><h3 className="font-bold text-lg flex items-center"><Calculator className="mr-2" /> Resumen</h3></div>
                <div className="p-6 space-y-6 flex-1 overflow-y-auto">
                    <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Cliente</label><div className="relative"><User className="absolute left-3 top-3 text-gray-400" size={18} /><input value={clientName} onChange={e => setClientName(e.target.value)} placeholder="Nombre del cliente..." className="w-full pl-10 p-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none font-bold text-gray-700" /></div></div>
                    <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1 flex justify-between"><span>Descuento</span><span className="text-blue-600 font-bold">{discountPercent}% OFF</span></label><input type="range" min="0" max="50" step="5" value={discountPercent} onChange={e => setDiscountPercent(parseInt(e.target.value))} className="w-full accent-blue-600 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" /><div className="flex justify-between text-[10px] text-gray-400 mt-1 px-1 font-bold"><span>0%</span><span>25%</span><span>50%</span></div></div>
                    <hr className="border-dashed" />
                    <div className="space-y-3 bg-gray-50 p-4 rounded-lg border border-gray-100"><div className="flex justify-between text-gray-600 text-sm"><span>Subtotal</span><span>$ {subtotal.toLocaleString()}</span></div>{discountPercent > 0 && <div className="flex justify-between text-green-600 font-bold text-sm"><span>Descuento ({discountPercent}%)</span><span>- $ {discountAmount.toLocaleString()}</span></div>}<div className="flex justify-between text-3xl font-black text-slate-800 pt-2 border-t border-gray-200"><span>Total</span><span>$ {total.toLocaleString()}</span></div></div>
                </div>
                <div className="p-4 bg-white border-t"><button onClick={handleSaveBudget} disabled={cart.length === 0 || !clientName} className="w-full bg-slate-800 text-white py-4 rounded-xl font-bold text-lg shadow-lg hover:bg-slate-900 transition-all flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed group">{cart.length > 0 && clientName ? <><Printer className="mr-2 group-hover:scale-110 transition-transform" /> Guardar y PDF</> : "Completa los datos..."}</button></div>
            </div>
        </div>
    );
};
