...
const BudgetPage = () => {
    ...
    // --- ESTADOS DE FILTROS Y ZOOM (NUEVOS) ---
    ...
    // Modificado para soportar el ordenamiento (sort_by)
    useEffect(() => {
                ...
                <div className="p-5 border-b border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 z-10 shrink-0">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 mb-2">
                        <div className="flex gap-2 w-full sm:w-auto">
                            <select className="flex-1 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg text-xs font-bold text-gray-600 dark:text-slate-300 outline-none focus:ring-2 focus:ring-yellow-400 p-2" value={selectedCat} onChange={e => setSelectedCat(e.target.value)}>
                                <option value="">Todas</option>
                                {categories.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}
                            </select>
                            <select className="flex-1 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg text-xs font-bold text-gray-600 dark:text-slate-300 outline-none focus:ring-2 focus:ring-yellow-400 p-2" value={sortBy} onChange={e => setSortBy(e.target.value)}>
                    ...
                    {manualResults.length === 0 && !manualTerm ? <div className="h-full flex flex-col items-center justify-center text-gray-300 dark:text-slate-700"><Search size={48} className="mb-2 opacity-50" /><p className="text-xs font-medium">Usa el buscador para agregar ítems</p></div> : manualResults.map(p => (
                        <div key={p.id} className="bg-white dark:bg-slate-800 p-3 rounded-xl border border-gray-200 dark:border-slate-700 mb-2 shadow-sm hover:border-yellow-400 dark:hover:border-yellow-600 transition-all group">
                            <div className="flex gap-3">
                                {/* IMAGEN CON ZOOM */}
                                <div
                                    className="w-12 h-12 bg-gray-100 dark:bg-slate-700 rounded-lg shrink-0 overflow-hidden border dark:border-slate-600 flex items-center justify-center relative cursor-zoom-in group/img"
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    {/* VARIANTES CON SEMÁFORO DE STOCK */}
                                    <div className="flex flex-wrap gap-1">
                                        {p.variantes.map(v => (
                                            <button
                                                key={v.id_variante}
                                                onClick={() => { if (useRealStock && v.stock === 0) return; addToCart(p, v); }}
                                                className={`text-[10px] border px-2 py-1 rounded transition-all font-bold flex items-center gap-1 ${v.stock === 0
                                                    ? `bg-red-50 text-red-600 border-red-200 dark:bg-red-900/20 dark:text-red-400 dark:border-red-800/50 ${useRealStock ? 'opacity-50 cursor-not-allowed' : 'hover:bg-red-100 dark:hover:bg-red-900/40'}`
                                                    : v.stock <= 5
                                            ...
                                            >
                                                {v.talle} <span className="opacity-70 text-[9px] border-l pl-1 border-current">{v.stock}</span>
                                            </button>
                                        ))}
                                    </div>
                                </div>
