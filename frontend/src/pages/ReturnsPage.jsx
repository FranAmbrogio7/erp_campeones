...
const ReturnsPage = () => {
    ...
    // --- CORRECCIÓN: FUNCIONES ELIMINAR (TACHITO) ---
    const removeItemIn = (uid) => {
        setItemsIn(prev => prev.filter(i => i.uid !== uid));
    };

    const removeItemOut = (uid) => {
        setItemsOut(prev => prev.filter(i => i.uid !== uid));
    };
    ...
    const handleProcess = async () => {
        ...
        // Validación de Pago si hay diferencia a favor de la tienda
        if (balance > 0 && !selectedPaymentMethod) {
            playSound('error');
            toast.error("⚠️ Selecciona un Método de Pago para cobrar la diferencia.");
            return;
        }

        if (!window.confirm(`¿Confirmar operación?\n${balance > 0 ? `Se registrará una venta por $${balance.toLocaleString()}` : 'Afectará stock y caja.'}`)) return;
        ...
        try {
            const res = await api.post('/returns/process', {
                items_in: itemsIn,
                items_out: itemsOut,
                // Enviamos el pago para que el backend cree la Venta
    ...
    return (
        <div className="p-4 h-full flex flex-col max-w-[1600px] mx-auto gap-4">
                        ...
                        <div className="p-6 overflow-y-auto flex-1 bg-gray-50">
                            {transactionResult.nota ? (
                                <div className="bg-white p-4 rounded-xl border border-blue-100 shadow-sm mb-6 flex justify-between items-center relative overflow-hidden">
                                    ...
                                    <div><p className="text-xs font-bold text-gray-400 uppercase">Nota de Crédito</p><p className="text-xl font-black text-gray-800 font-mono">{transactionResult.nota.codigo}</p></div>
                                    <div className="text-right"><p className="text-xs font-bold text-gray-400 uppercase">Saldo a Favor</p><p className="text-xl font-black text-blue-600">$ {transactionResult.nota.monto.toLocaleString()}</p></div>
                                ...
                                transactionResult.balance > 0 ? (
                                    <div className="bg-green-50 p-4 rounded-xl border border-green-200 text-center mb-6">
                                        <p className="text-green-800 font-bold text-lg">✅ Venta registrada por diferencia: ${transactionResult.balance.toLocaleString()}</p>
                                    </div>
                                ) : (
                                    <div className="bg-gray-100 p-3 rounded-lg text-center text-gray-500 text-sm font-bold mb-6">⚖️ Cambio sin diferencia económica</div>
                                )
                            )}
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="bg-white rounded-xl border border-green-100 shadow-sm overflow-hidden">
                                    <div className="bg-green-50 p-2 border-b border-green-100 flex items-center justify-center text-green-700 font-bold text-xs uppercase"><PackagePlus size={14} className="mr-2" /> Ingresó al Stock (+1)</div>
                                    <div className="p-2 space-y-2 max-h-40 overflow-y-auto">{transactionResult.itemsEntraron.map(i => (<div key={i.uid} className="flex items-center gap-2 p-2 rounded-lg bg-gray-50"><div className="w-8 h-8 bg-white rounded border flex items-center justify-center shrink-0">{i.imagen ? <img src={`${api.defaults.baseURL}/static/uploads/${i.imagen}`} className="w-full h-full object-cover" /> : <Shirt size={12} className="text-gray-300" />}</div><div className="min-w-0"><p className="text-xs font-bold text-gray-700 truncate">{i.nombre}</p><p className="text-[10px] text-gray-500">Talle: {i.talle}</p></div></div>))}</div>
                                </div>
                                <div className="bg-white rounded-xl border border-red-100 shadow-sm overflow-hidden">
                                    <div className="bg-red-50 p-2 border-b border-red-100 flex items-center justify-center text-red-700 font-bold text-xs uppercase"><PackageMinus size={14} className="mr-2" /> Salió del Stock (-1)</div>
                                    <div className="p-2 space-y-2 max-h-40 overflow-y-auto">{transactionResult.itemsSalieron.map(i => (<div key={i.uid} className="flex items-center gap-2 p-2 rounded-lg bg-gray-50"><div className="w-8 h-8 bg-white rounded border flex items-center justify-center shrink-0">{i.imagen ? <img src={`${api.defaults.baseURL}/static/uploads/${i.imagen}`} className="w-full h-full object-cover" /> : <Shirt size={12} className="text-gray-300" />}</div><div className="min-w-0"><p className="text-xs font-bold text-gray-700 truncate">{i.nombre}</p><p className="text-[10px] text-gray-500">Talle: {i.talle}</p></div></div>))}</div>
                                </div>
                            </div>
                        </div>
                        <div className="p-4 bg-white border-t flex gap-3">
                            {transactionResult.nota && <button onClick={reactToPrintFn} className="flex-1 bg-slate-800 text-white py-3 rounded-xl font-bold hover:bg-slate-900 flex items-center justify-center shadow-lg"><Printer size={18} className="mr-2" /> Imprimir Nota</button>}
                            <button onClick={() => setTransactionResult(null)} className="flex-1 bg-gray-100 text-gray-700 py-3 rounded-xl font-bold hover:bg-gray-200 border border-gray-200">Cerrar y Continuar</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="flex justify-between items-center mb-2 shrink-0">
                <div><h1 className="text-2xl font-black text-gray-800 flex items-center"><RefreshCcw className="mr-3 text-blue-600" /> Centro de Cambios</h1><p className="text-sm text-gray-500">Gestión de devoluciones y garantías.</p></div>
                {(itemsIn.length > 0 || itemsOut.length > 0) && <button onClick={() => { if (window.confirm("¿Borrar todo?")) { setItemsIn([]); setItemsOut([]); setSelectedPaymentMethod(null); } }} className="text-red-500 hover:text-red-700 font-bold text-sm bg-red-50 px-4 py-2 rounded-lg border border-red-100 transition-colors">Reiniciar</button>}
            </div>
            ...
            <div className="flex flex-col lg:flex-row gap-4 flex-1 overflow-hidden">
                ...
                <div className="flex-1 flex flex-col bg-white rounded-2xl shadow-sm border border-red-100 overflow-visible relative z-20">
                    ...
                    <div className="p-4 bg-red-50/50 border-b border-red-100">
                        <h3 className="font-bold text-red-800 flex items-center mb-3 text-sm uppercase tracking-wide"><ArrowLeft className="mr-2" size={18} /> Cliente Entrega (Devolución)</h3>
                        <div className="relative">
                            <form onSubmit={(e) => handleScan(e, 'IN')}><input ref={inputInRef} value={termIn} onChange={e => setTermIn(e.target.value)} className="w-full pl-9 pr-4 py-2.5 rounded-xl border-2 border-red-100 focus:border-red-400 focus:ring-4 focus:ring-red-50 outline-none transition-all font-bold text-gray-700 placeholder-red-300 text-sm" placeholder="Escanear..." autoFocus /><Search className="absolute left-3 top-3 text-red-300" size={16} /></form>
                            <SearchResultsDropdown results={resultsIn} type="IN" />
                        </div>
                    </div>
                    <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-gray-50/30">
                        {itemsIn.length === 0 ? <div className="h-full flex flex-col items-center justify-center text-red-200 opacity-60"><ArrowLeft size={48} className="mb-2" /><p className="font-bold text-sm">Escanea producto devuelto</p></div> : itemsIn.map((item) => (
                            <div key={item.uid} className="bg-white p-3 rounded-xl shadow-sm border border-red-100 flex justify-between items-center animate-fade-in-left">
                                <div className="flex items-center gap-3"><div className="w-10 h-10 bg-gray-100 rounded border shrink-0 overflow-hidden">{item.imagen ? <img src={`${api.defaults.baseURL}/static/uploads/${item.imagen}`} className="w-full h-full object-cover" /> : <Shirt size={16} className="text-gray-300 m-auto mt-2" />}</div><div><p className="font-bold text-gray-800 text-sm leading-tight">{item.nombre}</p><p className="text-[10px] text-gray-500 font-mono bg-red-50 inline px-1.5 py-0.5 rounded mt-1">Talle: {item.talle}</p></div></div>
                                <div className="flex items-center gap-3"><span className="font-bold text-red-600 text-sm">$ {item.precio.toLocaleString()}</span><button onClick={() => removeItemIn(item.uid)} className="text-gray-300 hover:text-red-500"><Trash2 size={16} /></button></div>
                            </div>
                        ))}
                    </div>
                    <div className="p-3 border-t border-red-100 bg-white flex justify-between items-center rounded-b-2xl"><span className="text-xs font-bold text-red-400 uppercase">Total Devolución</span><span className="text-xl font-black text-red-600">$ {totalIn.toLocaleString()}</span></div>
                </div>
                ...
                <div className="flex-1 flex flex-col bg-white rounded-2xl shadow-sm border border-green-100 overflow-visible relative z-20">
                    ...
                    <div className="p-4 bg-green-50/50 border-b border-green-100">
                        <h3 className="font-bold text-green-800 flex items-center mb-3 text-sm uppercase tracking-wide"><ArrowRight className="mr-2" size={18} /> Cliente Lleva (Nuevo)</h3>
                        <div className="relative">
                            <form onSubmit={(e) => handleScan(e, 'OUT')}><input ref={inputOutRef} value={termOut} onChange={e => setTermOut(e.target.value)} className="w-full pl-9 pr-4 py-2.5 rounded-xl border-2 border-green-100 focus:border-green-400 focus:ring-4 focus:ring-green-50 outline-none transition-all font-bold text-gray-700 placeholder-green-300 text-sm" placeholder="Escanear..." /><Search className="absolute left-3 top-3 text-green-300" size={16} /></form>
                            <SearchResultsDropdown results={resultsOut} type="OUT" />
                        </div>
                    </div>
                    <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-gray-50/30">
                        {itemsOut.length === 0 ? <div className="h-full flex flex-col items-center justify-center text-green-200 opacity-60"><ArrowRight size={48} className="mb-2" /><p className="font-bold text-sm">Escanea producto nuevo</p></div> : itemsOut.map((item) => (
                            <div key={item.uid} className="bg-white p-3 rounded-xl shadow-sm border border-green-100 flex justify-between items-center animate-fade-in-right">
                                <div className="flex items-center gap-3"><div className="w-10 h-10 bg-gray-100 rounded border shrink-0 overflow-hidden">{item.imagen ? <img src={`${api.defaults.baseURL}/static/uploads/${item.imagen}`} className="w-full h-full object-cover" /> : <Shirt size={16} className="text-gray-300 m-auto mt-2" />}</div><div><p className="font-bold text-gray-800 text-sm leading-tight">{item.nombre}</p><p className="text-[10px] text-gray-500 font-mono bg-green-50 inline px-1.5 py-0.5 rounded mt-1">Talle: {item.talle}</p></div></div>
                                <div className="flex items-center gap-3"><span className="font-bold text-green-600 text-sm">$ {item.precio.toLocaleString()}</span><button onClick={() => removeItemOut(item.uid)} className="text-gray-300 hover:text-red-500"><Trash2 size={16} /></button></div>
                            </div>
                        ))}
                    </div>
                    <div className="p-3 border-t border-green-100 bg-white flex justify-between items-center rounded-b-2xl"><span className="text-xs font-bold text-green-400 uppercase">Total Nuevos</span><span className="text-xl font-black text-green-600">$ {totalOut.toLocaleString()}</span></div>
                </div>
            </div>
        </div>
    );
